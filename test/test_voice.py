...
class TestVoiceService(unittest.TestCase):
    def test_call(self):
        res = service.call(
            callFrom="+254741126228", callTo=["+254741126228", "+254741126228"]
        )
        assert res["errorMessage"] != "None"

    def test_fetch_queued_calls(self):
        res = service.fetch_queued_calls(phone_number="+254741126228")

        assert res["status"] == "Success"

    def test_media_upload(self):
        res = service.media_upload(
            phone_number="+254741126228", url="https://aksalj.com"
        )
        assert (
            res
